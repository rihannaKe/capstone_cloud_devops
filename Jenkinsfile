    ...
    stage('Deploying') {
      ...
      dir ('./') {
        withAWS(credentials: 'demo-ecr-credentials', region: 'us-east-2') {
            ...
            sh "kubectl get pods"
            sh "aws cloudformation create-stack --stack-name udacity-capstone-infra --template-body file://aws/infrastructure.yml --parameters file://aws/infrastructure_parameters.json --capabilities CAPABILITY_IAM"
            sh "aws cloudformation create-stack --stack-name udacity-capstone-infra --template-body file://aws/eks.yml --parameters file://aws/eks_parameters.json --capabilities CAPABILITY_IAM"
