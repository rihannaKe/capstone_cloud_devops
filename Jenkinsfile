    ...
    stage('Deploying') {
      ...
      dir ('./') {
        withAWS(credentials: 'demo-ecr-credentials', region: 'us-east-2') {
            ...
            sh "kubectl get pods"
            sh "aws cloudformation create-stack --stack-name udacity-cap-nodes --template-body file://aws/worker_nodes.yml --parameters file://aws/worker_nodes_parameters.json --capabilities CAPABILITY_IAM"
        }
      }
    }
    stage("Cleaning up") {
